...
module.exports = async function (api, createLine) {
        ...
        const controller = await require(path.join(__dirname, '..', '..', 'core/database/controller/index.js'))(api); // data is loaded here
        ...
        if (api && global.GoatBot.config.database.autoSyncWhenStart == true) {
                ...
                try {
                        ...
                        api.setOptions({
                                logLevel: (global.GoatBot.fcaConfig?.optionsFca?.logLevel) || (global.GoatBot.config.optionsFca?.logLevel) || "warn"
                        });
                }
        }
        // ————————————— ——————————— ———————————— ——————————— //
