                                ...
                                case "update": {
                                        ...
                                        switch (databaseType) {
                                                ...
                                                case "sqlite": {
                                                        ...
                                                        let dataUpdated;
                                                        if (user) {
                                                                dataUpdated = (await user.update(dataWillChange)).get({ plain: true });
                                                        } else {
                                                                const merged = { ...global.db.allUserData[index], ...dataWillChange, userID };
                                                                dataUpdated = (await userModel.create(merged)).get({ plain: true });
                                                        }
                                                        global.db.allUserData[index] = dataUpdated;
                                                        return _.cloneDeep(dataUpdated);
                                                }
                        ...
                        try {
                                ...
                                if (!userInfo) {
                                        throw new CustomError({
                                                ...
                                                message: `Could not fetch user info for userID "${userID}"`
                                        });
                                }
                                let userData = {
                                        userID,
                                        name: userInfo.name || String(userID),
