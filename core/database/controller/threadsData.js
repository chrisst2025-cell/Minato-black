                                ...
                                case "update": {
                                        ...
                                        if (Array.isArray(path) && Array.isArray(threadData)) {
                                                path.forEach((p, index) => {
                                                        const key = p.split(".")[0];
                                                        dataWillChange[key] = _.cloneDeep(oldThreadData[key]);
                                        ...
                                        switch (databaseType) {
                                                ...
                                                case "sqlite": {
                                                        let thread = await threadModel.findOne({ where: { threadID } });
                                                        let dataUpdated;
                                                        if (thread) {
                                                                dataUpdated = (await thread.update(dataWillChange)).get({ plain: true });
                                                        } else {
                                                                const merged = { ...global.db.allThreadData[index], ...dataWillChange, threadID };
                                                                dataUpdated = (await threadModel.create(merged)).get({ plain: true });
                                                        }
                                                        global.db.allThreadData[index] = dataUpdated;
                                                        return _.cloneDeep(dataUpdated);
                                                }
                        ...
                        try {
                                ...
                                if (!threadInfo) {
                                        throw new CustomError({
                                                ...
                                                message: `Could not fetch thread info for threadID "${threadID}"`
                                        });
                                }
